...
class AtomFact:
    ...
    rank against until a measured number backfills it. ``storage`` is
    per-thread fragment occupancy in bytes; ``resource`` is the required
    thread-scope footprint, e.g. ``{"lane": 32}`` for one warp.
    ``is_async`` marks an asynchronous instruction. ``atom`` is the
    target's own realized descriptor, carried through so a later
    fill/codegen stage need not re-resolve it from ``shape``/``dtype``.
    """
