...
Each occurrence holds every position its result placement names for one
CTA-local duration. A position runs one occurrence at a time and an occurrence
waits for what it reads, so the program's own placement says what overlaps.
Nothing is searched. Where the buffers sit belongs to ``memory``.
