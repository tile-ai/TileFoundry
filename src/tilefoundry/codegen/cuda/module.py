...
def emit_cuda_module(
    ...
    for fn in expanded_fns:
        owner = owning_module(module, fn) or module
        ctx = CudaCodegenContext(target, owner.effective_topologies())
    ...
    for f in all_fields[1:]:
        if f.launch != base.launch:
            raise ValueError(
                "emit_cuda_module: kernels disagree on launch topology — "
                f"{base.kernel_name!r} has {base.launch!r} "
                f"but {f.kernel_name!r} has {f.launch!r}"
            )
    specs = _topology_dim_specializations(base.launch.grid, base.launch.block)
    ...
    source = render(
        ...
        kernels="\n".join(kernel_texts),
        dynamic_cta=base.launch.grid[0] is None,
