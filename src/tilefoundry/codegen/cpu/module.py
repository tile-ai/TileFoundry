...
from tilefoundry.codegen.signature import LaunchSignature
...
def _static_smem(value) -> int:
    ...
    covers the projected grid/block geometry only; a dynamic shared-memory size expression
    is not supported.
    """
...
def _lower_launches(entry: PrimFunction, evaluates, module):
    ...
    for index, (evaluate, device_fn, hidden, dev_to_host) in enumerate(bindings):
        ...
        owner = owning_module(module, device_fn) or module
        launch = LaunchSignature.from_topologies(owner.effective_topologies())
        call_args = [_call_arg(p, host_names, hidden) for p in dev_params] + [
            *(str(d) for d in (*launch.grid, *launch.block)),
...
def _lower_dispatch(entry: PrimFunction, module, *, callee_name, subject, variants, calls):
    ...
    for idx, (variant, call) in enumerate(zip(variants, calls)):
        ...
        owner = owning_module(module, variant) or module
        launch = LaunchSignature.from_topologies(owner.effective_topologies())
        if launch.grid[0] is None:
            raise ValueError(
        ...
        shim_args += [str(d) for d in (*launch.grid, *launch.block, 0)]
