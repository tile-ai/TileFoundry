...
def flat_reshape_map(old_shape: tuple, new_shape: tuple) -> "isl.map":
    ...
    for i, extent in enumerate(old_mid):
        ...
        # i==0 (outermost mid axis): floor(f/stride) < extent already (f is
        # bounded by the middle's own total) -- mod would be a no-op, skip for
        # a cleaner expression. i>0: strips the more-significant digits an
        # outer axis already consumed.
