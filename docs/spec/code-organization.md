...
| `ir/visitor.py` | [visitor-mutator](./visitor-mutator.md) | `ExprFunctor` / `ExprVisitor` / `ExprWalker` / `ExprCollector` / `ExprCloner` / `StmtVisitor` / `StmtMutator` / `StmtExprMutator`, plus the canonical `PrimFunction` walk and rewrite entries. |
...
| `analysis/visitor.py` | [analysis](./analysis.md) | The per-call `AnalyzeContext` and its identity-preserving `StructuralMemo`: one normalized-HIR walk records lexical scopes, definition order, producers, users, loop trip counts, and loop variance for every analysis family to share. |
| `analysis/walk.py` | [analysis](./analysis.md) | How every family reads the authored program: SSA-DAG traversal order, the values of a function, the reachable call graph, a type's tensor leaves and byte size per storage level, and the execution-domain or topology extents its layouts imply. Names no Target — two families cannot disagree about the program they measured. |
| `analysis/poly/` | [analysis](./analysis.md) | The polyhedral subpackage: `model.py` owns `TileUnit` / `TileGraph`, `extract.py` builds that graph from normalized HIR, `access.py` owns its time/access queries, and `errors.py` owns `ExtractError`; `__init__.py` is the retained public import surface. |
| `analysis/poly/affine.py` | [analysis](./analysis.md) | The shared loop-affine term parser used by poly extraction and authored-loop footprint binding, including constant loop strides and bounded invariant offsets. It does not introduce a second affine graph representation. |
...
| `analysis/check.py` | [analysis](./analysis.md) | The shared authored-program gate for analysis and scheduling: authored-type re-derivation, authored validation, call-context validation, and checker-specific input checks. Established once per public call rather than per family, before analysis or scheduling makes placement decisions. |
...
| `analysis/performance.py` | [analysis](./analysis.md) | The `performance` family: an occurrence plan projected from `StructuralMemo`, followed by flat timeline entries and per-scope makespans, scaled by a parallel capacity. It introduces no second scope tree. |
