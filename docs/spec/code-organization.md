...
| `schedule/` | [schedule](./schedule.md) | The public Schedule boundary in `schedule/__init__.py` -- the `schedule()` operation and its immutable options, result, plan base, and objective report -- plus the stages an algorithm composes its solve from: `kernel_schedule.py` (schedule-tree construction and band operations), `select_atoms.py` (atom selection and the facts it records), `render.py` (holed skeleton, swimlane, hole contracts). Concrete algorithms and their Plan types live with the Target they schedule. |
