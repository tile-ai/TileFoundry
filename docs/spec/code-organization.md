...
| `analysis/` | [analysis](./analysis.md) | Fact layer over typed HIR: `poly.py` (the polyhedral model — `extract` / `TileGraph` and the facts measured over a time relation), `atom_facts.py` (`AtomFact`), and one module per analysis family. The `Analysis` service protocol and the compact public surface live in `analysis/__init__.py`; per-target atom catalogues and Facts projections live with their owning Target. |
...
| `registry.py` | [analysis](./analysis.md) | The shared exact-key `AlgorithmRegistry`: algorithms bound to a `(Target concrete type, selector)` pair, with separate instances per dispatching stage rather than one differently shaped system per stage. Generic — it names no concrete target and applies no subclass or default fallback. |
...
| `analysis/registry.py` | [analysis](./analysis.md) | Analysis registration: an algorithm's selector, declared dependencies, and the Metadata types it owns. |
...
| `analysis/analyzer.py` | [analysis](./analysis.md) | The flag-selected authored-HIR entry, as a facade that maps flags onto root selectors and renders the records the families left. Measures nothing; scheduled for removal with the flags themselves. |
