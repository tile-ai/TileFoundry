...
| `target/` | [target](./target.md) | Compilation Target classes, class registration, service selection, and architecture/device facts: `Architecture` / `Device` / `Target` / `CudaTarget` / `CpuTarget` / `register_target` / `registered_targets`. Authored code constructs Target values; there is no string resolver. |
| `target/registration.py` | [target](./target.md) | The sole Target extension registry: no-argument `@register_target`, idempotent logical-provider handling, conflicts, and the read-only registered-class view. It never constructs a Target. |
...
| `analysis/registry.py` | [analysis](./analysis.md) | The immutable `Analyzer` descriptor: selector, run callable, dependencies, and owned Metadata types. Targets select these descriptors; this module holds no Target dispatch table. |
...
| `analysis/facts.py` | [analysis](./analysis.md) | The narrow Facts aggregates the families declare — the memory hierarchy graph, the throughput rates, and the parallel capacity. It is the record of how much hardware each measurement rests on, and names no backend. |
...
| `schedule/registry.py` | [schedule](./schedule.md) | The immutable `Scheduler` descriptor selected by `Target.get_scheduler`; it holds no Target dispatch table or registration decorator. |
...
| `target/<backend>/schedule.py` | [target](./target.md) | One backend's scheduling algorithms and immutable Scheduler descriptors. This is where that backend's private problem construction, solve, and Plan export are composed; its Target class selects them. |
...
| `target/facts.py` | [target](./target.md) | Shared validation for values returned by `Target.get_facts`: the requested frozen-dataclass shape and returned type. It holds no projection registry. |
...
| `codegen/registry.py` | [codegen](./codegen.md) | The immutable `CodeGenerator` descriptor and source-order grouping by equal Target values. It rejects multiple unequal CUDA Target groups before emission and holds no emitter registry. |
