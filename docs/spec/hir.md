...
## 2. Model dataflow representation

Real model kernels use ordinary HIR `Function` values and existing primitive
Ops. Routed expert computation is represented as explicit tensor dataflow:
`Gather`, `Cast`, `Reshape`, elementwise arithmetic, `MatMul`,
`SwiGLU`-equivalent scalar Ops, weighting, and `Reduce`. A semantic
expert-grouping or quantization Op is not part of this representation.

The routed expert axis remains explicit through weight gathering,
dequantization, batched projections, activation, and weighting. For the
DeepSeek decode contract its shape is `(1, 6, 4096)`, with routed banks
`(256, 2048, 4096)`, `(256, 2048, 4096)`, and `(256, 4096, 2048)`;
the routed weights use `f4e2m1` and one `f8e8m0` scale per 32-value
innermost block. Shared FP8 projections use
`(2048, 4096)`, `(2048, 4096)`, and `(4096, 2048)`, with `128 x 128`
weights in `fp8e4m3` and `f8e8m0` scale blocks. The expert axis is
removed only by the weighted sum reduction.

An authored root may carry an explicit CUDA target and a static `cta`
topology upper bound. Helper Functions omit target and program-topology
ownership. Hard `where(...)` metadata may constrain the pre-reduction expert
axis to a CTA `Split` and the combined result to CTA `Broadcast`; these
constraints do not alter the tensor type or introduce an IR node.

Online-softmax state is represented by a single nested `GridRegionExpr`
whose carried values are the running `m`, `l`, and `o` tensors. A static
region retains positive `start`, `extent`, and `step` values in the loop
domain, and nested regions compose recursively without Python unrolling.
