...
class MiniCPM3_4B_DecoderLayer:
    ...
    def input_rms_norm(
        ...
        # `MiniCPMRMSNorm` rounds the normalised activation to the input dtype before
        # the learned scale multiplies it; `tf.rms_norm` keeps f32 through that
        # multiply. On bf16 the two differ in the last bit.
...
class MiniCPM3_4B:
    ...
    def prepare_inputs_for_generation(self, input_ids, step, caches, device=None):
        ...
        # `MiniCPM3Attention.scaling` is `qk_head_dim ** -0.5` -- the assembled
        # 96-wide head, nope and rope together, not `config.head_dim` (32, the
        # rotary slice alone). The two differ by sqrt(3) and the model still
        # reads fluently at the wrong one, so this is checked against HF's own
        # `layer.self_attn.scaling` (tools/hf_agree.py) rather than eyeballed:
        # 96 puts the step's logits at cosine 0.99998 of HF's, 32 at 0.99879.
        scale = torch.full(
