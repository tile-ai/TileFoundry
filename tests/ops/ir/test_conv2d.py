...
def test_grouped_conv2d_matches_torch_reference() -> None:
    ...
    bias = torch.randn(6)
    expected = F.conv2d(input_, weight, bias, stride=(2, 1), padding=(1, 0), groups=2)
...
def test_conv2d_relation_propagates_surviving_splits(input_, weight, bias, attrs, targets) -> None:
...
SHARD_ERROR_CASES = [
    ...
    TypeInferCase(
        ...
        (
            ...
            make_shard_tensor_type((6, 2, 3, 3), mesh=_MESH, attrs=(Split(1),)),
...
COST_CASES = [
    ...
    CostCase(
        ...
        _OP,
        (
            make_shard_tensor_type((2, 4, 7, 7), mesh=_CTA_MESH, attrs=(Split(0),)),
            _W,
            _BIAS,
        ),
    ...
    CostCase(
        ...
        Conv2D(stride=(1, 1), padding=(0, 0), dilation=(1, 1), groups=1),
        (
            make_shard_tensor_type((1, 4, 8, 8), mesh=_CTA_MESH, attrs=(Split(1),)),
            make_shard_tensor_type((4, 4, 3, 3), mesh=_CTA_MESH, attrs=(Split(1),)),
            make_shard_tensor_type((4,), mesh=_CTA_MESH, attrs=(Partial("sum"),)),
        ),
