...
_PSUM = make_shard_tensor_type(
    (8, 16), mesh=_PARTIAL_MESH, attrs=(Partial("sum"),), dtype=DType.f32
)
_PMAX = make_shard_tensor_type(
    (8, 16), mesh=_PARTIAL_MESH, attrs=(Partial("max"),), dtype=DType.f32
)
...
def test_plain_reduce_layout_describes_the_reduced_result():
    ...
    result = infer_call(Reduce(axes=(-1,), keepdim=False, kind=ReduceKind.SUM), source)

    assert result.shape == (1, 16, 1024)
    assert result.layout == Layout(shape=(1, 16, 1024), strides=(16384, 1024, 1))
