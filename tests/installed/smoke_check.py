...
def test_a_whole_module_is_checked_against_an_expected_output_file(tf, twin, tmp_path) -> None:
...
def test_real_weights_come_from_the_checkpoint_and_activations_are_drawn(
    ...
    save_file({"w": torch.linspace(0.5, 2.0, 168)}, str(tmp_path / "model.safetensors"))
        ...
        "check",
        f"{twin}:WeightedRootTwin.scaled",
        "--inputs",
        "real",
        "--ckpt",
        str(tmp_path),
...
def test_a_nested_twin_is_reached_through_the_child_it_is_declared_under(tf, twin) -> None:
