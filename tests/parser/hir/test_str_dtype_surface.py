...
"""
    string_ir = parse_script(_dedent(string_dtype))
    descriptor_ir = parse_script(_dedent(descriptor_dtype))
...
"""
    assert as_script(parse_script(_dedent(string_kind))) == as_script(
        parse_script(_dedent(enum_kind))
    )
...
@pytest.mark.parametrize(
    ("src", "message"),
    [
        (_BAD_CALL_DTYPE, r"DType: unknown value 'float32'"),
        (_BAD_ANNOTATION_DTYPE, r"DType: unknown value 'float32'"),
        (_BAD_REDUCE_KIND, r"ReduceKind: unknown value 'plus'"),
    ],
    ids=["call-dtype", "annotation-dtype", "reduce-kind"],
)
def test_an_unknown_surface_string_names_the_type_it_failed(src: str, message: str) -> None:
