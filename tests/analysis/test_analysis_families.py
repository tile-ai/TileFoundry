...
_BLOCK_WIDTH = 128
_BLOCK_ROWS = 256
_BLOCK_SEQ = DimVar("seq_len", 0, 8193)


@module(entry="walk", target=_H200, topologies=(Topology("cta", _BLOCK_GRID),))
class _BlockWalk:
    """A block of layers in an inner loop, then one more at the block level.

    Which is what every layered model is, and the shape that used to make
    `performance` raise `KeyError`: the value the outer-level call reads was
    produced two loops down, inside the inner loop's own callee, so the
    representative the timeline resolved for it named a node that the outer
    scope does not schedule.
    """

    @func
    def stage(
        x: Tensor[(_BLOCK_STRIPS, _BLOCK_SEQ, _BLOCK_WIDTH), "bf16"],
        out: Tensor[(_BLOCK_STRIPS, _BLOCK_SEQ, _BLOCK_WIDTH), "bf16"],
    ) -> Tensor[(_BLOCK_STRIPS, _BLOCK_SEQ, _BLOCK_WIDTH), "bf16"]:
        with Mesh(("cta",), layout=(_BLOCK_STRIPS, 8), names=("strip", "tile")) as mesh:
            acc = out
            for start in tile(_BLOCK_SEQ, _BLOCK_ROWS):
                base = start + 0
                held = tf.reshard(
                    x[:, base : base + _BLOCK_ROWS, :],
                    (
                        _BLOCK_STRIPS @ mesh.strip,
                        _BLOCK_ROWS @ mesh.tile,
                        _BLOCK_WIDTH,
                    ),
                    "smem",
                )
                acc = tf.insert_slice(acc, held + held, (0, base, 0))
            return acc

    @func
    def walk(
        x: Tensor[(_BLOCK_STRIPS, _BLOCK_SEQ, _BLOCK_WIDTH), "bf16"],
        first: Tensor[(_BLOCK_STRIPS, _BLOCK_SEQ, _BLOCK_WIDTH), "bf16"],
        second: Tensor[(_BLOCK_STRIPS, _BLOCK_SEQ, _BLOCK_WIDTH), "bf16"],
    ) -> Tensor[(_BLOCK_STRIPS, _BLOCK_SEQ, _BLOCK_WIDTH), "bf16"]:
        with Mesh(("cta",), layout=(_BLOCK_GRID,), names=("unit",)) as _mesh:
            carried = x
            for _block in range(1):
                for _layer in range(2):
                    carried = stage(carried, first)
                carried = stage(carried, second)
            return carried


def test_a_block_of_layers_followed_by_one_more_is_scheduled() -> None:
    """A producer the scope cannot schedule is not a predecessor of it.

    The last call reads what the inner loop left, so its producer is hosted two
    nodes deeper than the scope the call sits in. There is no occurrence of that
    scope standing for it and there does not need to be: the inner loop ended
    before this scope's origin, which is what the scope is laid out from. What
    must not happen is resolving it to a node the scope does not hold and then
    looking that node up.
    """
    walk = next(function for function in _BlockWalk.functions if function.name == "walk")
    report = analyze(_BlockWalk, walk, analysis="performance", dims={"seq_len": 8192})

    summary = get_metadata(report.function, PerformanceSummaryMetadata)
    assert summary is not None
    assert summary.timeline.start_ns == 0
