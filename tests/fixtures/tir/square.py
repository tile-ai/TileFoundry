...
class TirSquare:
    ...
    def square_device(x: Tensor[(_N,), "f32"]):
        pass

    @square_device.specialize(DimVarRangePat("N", 1, 128))
    def square_small(x: Tensor[(_N,), "f32"]):
    ...
    @square_device.specialize(DimVarRangePat("N", 128, 256))
    def square_large(x: Tensor[(_N,), "f32"]):
        with Mesh((Topology("thread", 128),), Layout((128,), (1,))) as thread:
            view = T.tensor_view(x, layout=ShardLayout(layout=Layout(shape=(128,), strides=(1,)), attrs=(S(0),), mesh=Mesh(topologies=(Topology(name="thread", size=128),), layout=Layout(shape=(128,), strides=(1,)), names=())))
    ...
    def square_host(x: Tensor[(_N,), "f32"]):
