...
class TirRmsnorm:
    ...
    def rmsnorm_device(
        x: Tensor[(1, 128), "f32"], weight: Tensor[(128,), "f32"], out: Tensor[(1, 128), "f32"]
    ):
        with Mesh((Topology("thread", 1),), Layout((1,), (1,)), names=("t",)) as thread:
            x_view = T.tensor_view(T.ptr_of(x), layout=((1, 128), (128, 1), {thread.t @ B()}))
            weight_view = T.tensor_view(T.ptr_of(weight), layout=((128,), (1,), {thread.t @ B()}))
            out_view = T.tensor_view(T.ptr_of(out), layout=((1, 128), (128, 1), {thread.t @ B()}))
