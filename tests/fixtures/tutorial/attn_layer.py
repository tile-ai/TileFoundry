...
def _command(
    ...
    *,
    ...
    return "\n".join(
        (
            f"tilefoundry analyze attn_layer.py:{stage} \\",
...
def _authoring_source() -> str:
    """Return the original program section, without the Markdown generator."""
    ...
    tree = ast.parse(source)
        ...
        and any(
            ...
            and target.id == "gqa_decode_cache_prepared"
...
def render_markdown() -> str:
    ...
    report_body = [
        ...
        "",
        "This page is generated by executing the tutorial source file.",
        "The six Modules and this page therefore have one source: change the embedded program,",
        ...
        "|---|---|---|",
        f"| 0 | {_stage_label('Stage0_Naive')} | one unsplit CTA, open `ctx_len` |",
        f"| 1 | {_stage_label('Stage1_Specialized')} | dispatch on a measured range |",
        f"| 2 | {_stage_label('Stage2_Sharded')} | split query heads across CTAs |",
        f"| 3 | {_stage_label('Stage3_Fused')} | split the KV scan and combine online state |",
        f"| 4 | {_stage_label('Stage4_WeightPrepared')} | stage projection weights by output slice |",
        f"| 5 | {_stage_label('Stage5_CachePrepared')} | stream cache blocks through smem |",
        "",
        ...
        *sweep_rows,
        ...
        "",
        f"The full-cache sharded program in {_stage_label('Stage2_Sharded')} places one local "
        ...
        "",
        f"{_stage_label('Stage1_Specialized')} expresses the dispatch as two half-open "
        ...
        "",
        f"At long context, the full-cache form is the wrong residency choice. {_stage_label('Stage3_Fused')} "
        ...
        "Runtime `Module.load` and a weight converter are a different contract; the workflow "
        "explanation is in [migrate](migrate.md).",
        "",
        "## 6. Stream the KV cache",
        "",
        f"{_stage_label('Stage5_CachePrepared')} leaves the static projection weights in their ordinary "
        ...
        "|---|---|",
        f"| `@module(entry/target/topologies)` | {_stage_label('Stage0_Naive')} |",
        f"| `@func`, `Tensor`, `ConstTensor`, `DimVar` | {_stage_label('Stage0_Naive')} |",
        f"| `pass` prototype and `@f.specialize(DimVarRangePat)` | {_stage_label('Stage1_Specialized')} |",
        f"| single `Mesh`, shard sugar `X @ m.axis`, `reshard` to smem/gmem | {_stage_label('Stage2_Sharded')} |",
        f"| split-K worker mesh and online softmax state | {_stage_label('Stage3_Fused')} |",
        f"| weight staging and output gather | {_stage_label('Stage4_WeightPrepared')} |",
        f"| cache update, block scan, `matmul`, `rope`, `reduce`, `cast` | {_stage_label('Stage5_CachePrepared')} |",
