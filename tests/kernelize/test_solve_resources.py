...
def test_solve_resources_picks_atom_places_and_rings_end_to_end():
    ...
    a default duration, every written buffer (h, y) gets a ring depth,
    ...
    assert solved.ring, "TileGraph.ring must be non-empty: buffer -> depth"
...
def test_default_target_resolution_matches_explicit_cuda_target():
    ...
    assert implicit.decisions["statements"]["MM"]["atom"] == "SM80_16x8x16_F32BF16BF16F32_TN"
    assert explicit.decisions["statements"]["MM"]["atom"] == "SM80_16x8x16_F32BF16BF16F32_TN"
...
def test_empty_tile_graph_raises_clear_error():
    ...
    with pytest.raises(SolveResourcesError):
        solve_resources(empty_tg, target="cuda")
