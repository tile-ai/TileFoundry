"""The complete MiniCPM3-4B decoder at the real shape: loads ``model/decoder.py``
...
``n_layers`` distinct Modules rather than one Module referenced 62 times. That
...
def _layer(index: int):
    ...
    return loaded.MiniCPM3_4B.renamed(f"layer{index}")
...
def build_decoder(shape=REAL):
    ...
    return loaded.MiniCPM3_4B_Decoder
