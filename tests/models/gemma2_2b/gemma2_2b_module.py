...
Four Gemma-2-specific things to note (see ``tests/models/gemma2_2b/common.py``
...
- ``Gemma2RMSNorm`` is ``normed * (1.0 + weight)``; ``tf.rms_norm`` is
  ...
  is expected pre-adjusted by the caller (``common.rms_gamma``) — the kernel
...
from tests.models.gemma2_2b.common import (
    ATTN_SOFTCAP,
    DT,
    GQA_GROUP,
    HEAD_DIM,
...
class Gemma2_2B:
    ...
    def self_attention(
        ...
        q = tf.reshape(tf.matmul(hidden, w_q), new_shape=(1, S_CAP, NUM_Q_HEADS, HEAD_DIM))
        k = tf.reshape(tf.matmul(hidden, w_k), new_shape=(1, S_CAP, NUM_KV_HEADS, HEAD_DIM))
        v = tf.reshape(tf.matmul(hidden, w_v), new_shape=(1, S_CAP, NUM_KV_HEADS, HEAD_DIM))
        ...
        # attn_logit_softcapping: ATTN_SOFTCAP * tanh(scores / ATTN_SOFTCAP),
    ...
    def decoder_layer(
        ...
        scale: Tensor[(1, 1, 1, 1), DT],
        w_o: Tensor[(1, Q_PROJ, HIDDEN), DT],
        gamma_post_attn: Tensor[(HIDDEN,), DT],
        gamma_pre_ff: Tensor[(HIDDEN,), DT],
        w_gate: Tensor[(1, HIDDEN, INTERMEDIATE), DT],
        w_up: Tensor[(1, HIDDEN, INTERMEDIATE), DT],
        w_down: Tensor[(1, INTERMEDIATE, HIDDEN), DT],
        gamma_post_ff: Tensor[(HIDDEN,), DT],
    ) -> Tensor[(1, S_CAP, HIDDEN), DT]:
