...
name, stored form, or both. The model declarations remain the contract that the
loaded values must satisfy.
"""
...
def hf_alias(config: Qwen3Config) -> dict[str, object]:
    """Canonical names -> published Qwen3 checkpoint names for *config*."""
    return {
...
def hf_layout_only(config: Qwen3Config) -> dict[str, Preprocessed]:
