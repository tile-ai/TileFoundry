...
def test_the_complete_decoder_matches_hugging_face() -> None:
    """Every layer, in order, plus the final norm."""
    model, hidden_ctx, hidden_new, caches, weights, (cos, sin, pos_ids, scale) = _drawn()
...
def test_every_layer_returns_its_own_cache_entry() -> None:
...
def test_a_stack_that_is_wrongly_ordered_is_caught(description) -> None:
...
def test_a_wrong_final_norm_is_caught() -> None:
