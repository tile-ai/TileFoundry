...
driven by HF ``Qwen2Attention`` (see ``common.py`` module docstring):
...
from tests.models.qwen2_5_1_5b.common import (
    DT,
    GQA_GROUP,
    HEAD_DIM,
    HIDDEN,
...
class Qwen2_5_1_5B:
    ...
    def self_attention(
        ...
        bias_q: Tensor[(Q_PROJ,), DT],
        w_k: Tensor[(1, HIDDEN, KV_PROJ), DT],
        bias_k: Tensor[(KV_PROJ,), DT],
        w_v: Tensor[(1, HIDDEN, KV_PROJ), DT],
        bias_v: Tensor[(KV_PROJ,), DT],
        ...
        # prefill — no KV cache — so the mask is plain [S_CAP, S_CAP] causal
        # (cur_pos==0).
        hidden_norm = input_rms_norm(hidden, gamma_in)
        q_lin = tf.add(tf.matmul(hidden_norm, w_q), tf.reshape(bias_q, new_shape=(1, 1, Q_PROJ)))
        k_lin = tf.add(tf.matmul(hidden_norm, w_k), tf.reshape(bias_k, new_shape=(1, 1, KV_PROJ)))
        v_lin = tf.add(tf.matmul(hidden_norm, w_v), tf.reshape(bias_v, new_shape=(1, 1, KV_PROJ)))
        q = tf.reshape(q_lin, new_shape=(1, S_CAP, NUM_Q_HEADS, HEAD_DIM))
        k = tf.reshape(k_lin, new_shape=(1, S_CAP, NUM_KV_HEADS, HEAD_DIM))
        v = tf.reshape(v_lin, new_shape=(1, S_CAP, NUM_KV_HEADS, HEAD_DIM))
