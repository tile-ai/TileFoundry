...
its first sliding layer, not the whole causal LM. The tree above it -- embedding,
43 decoder layers, MoE, final norm, head -- is a real end-to-end path with a real
checkpoint pipeline behind it (`test_causal_lm_e2e.py`), and it is a tree of
Modules walked by orchestration methods rather than a Module of Functions: a
`ModelCase` names one Module and analysis and scheduling select Functions of that
one Module, so naming the root would put every kernel that does arithmetic out of
reach and report the model as three norms and an add.
...
CASE = ModelCase(
    ...
    source=MODELS_ROOT / "deepseek_v4_flash" / "model" / "attention.py",
    entry="DeepseekV4Attention",
