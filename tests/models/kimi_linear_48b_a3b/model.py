...
from tests.models.kimi_linear_48b_a3b.config import REAL, KimiLinearConfig
...
def build_kimi_linear_48b_a3b(config: KimiLinearConfig):
    ...
    _QK = config.qk_head_dim              # 192 = 128 nope + 64 rope
    ...
    _KH = config.kda_num_heads                # 32
    _KD = config.kda_head_dim             # 128
    _KP = config.kda_proj                 # 4096
    _W = config.short_conv_kernel_size                # 4
    _WS = _W - 1                          # 3 stored positions

    _E = config.num_experts
    _TOPK = config.num_experts_per_token
    _MI = config.moe_intermediate_size
    _SI = config.shared_intermediate
    ...
    class KimiMla:
        ...
        @func
        def mla_attention(
            hidden: Tensor[(1, S, config.hidden_size), config.dtype],
            gamma_in: Tensor[(config.hidden_size,), config.dtype],
            w_q: Tensor[(1, config.hidden_size, config.q_proj), config.dtype],
            w_kv_a: Tensor[(1, config.hidden_size, config.kv_a_proj), config.dtype],
            gamma_kv_a: Tensor[(config.kv_lora_rank,), config.dtype],
            w_kv_b: Tensor[(1, config.kv_lora_rank, config.kv_b_proj), config.dtype],
            cos_cache: Tensor[(config.max_pos, config.qk_rope_head_dim), config.dtype],
            sin_cache: Tensor[(config.max_pos, config.qk_rope_head_dim), config.dtype],
            pos_ids: Tensor[(S,), "i32"],
            k_cache: Tensor[(1, C, config.num_attention_heads, config.qk_head_dim), config.dtype],
            v_cache: Tensor[(1, C, config.num_attention_heads, config.v_head_dim), config.dtype],
            scale: Tensor[(1, 1, 1, 1), config.dtype],
            w_o: Tensor[(1, config.v_proj, config.hidden_size), config.dtype],
        ):
            ...
            # append.
            hn = tf.rms_norm(hidden, gamma_in, eps=_EPS)
    ...
    class KimiKda:
        ...
        @func
        def short_conv(
            x: Tensor[(1, S, config.kda_proj), config.dtype],
            conv_w: Tensor[(config.short_conv_kernel_size, config.kda_proj), config.dtype],
            conv_state: Tensor[(1, _WS, config.kda_proj), config.dtype],
        ):
        ...
        @func
        def l2_normalize(
            x: Tensor[(1, S, config.kda_num_heads, config.kda_head_dim), config.dtype],
        ) -> Tensor[(1, S, config.kda_num_heads, config.kda_head_dim), config.dtype]:
        ...
        @func
        def kda_gate(
            hidden_norm: Tensor[(1, S, config.hidden_size), config.dtype],
            w_f_a: Tensor[(1, config.hidden_size, config.kda_head_dim), config.dtype],
            w_f_b: Tensor[(1, config.kda_head_dim, config.kda_proj), config.dtype],
            dt_bias: Tensor[(config.kda_proj,), config.dtype],
            a_log: Tensor[(config.kda_num_heads,), config.dtype],
        ) -> Tensor[(1, S, config.kda_num_heads, config.kda_head_dim), config.dtype]:
        ...
        def kda_attention(
            hidden: Tensor[(1, S, config.hidden_size), config.dtype],
            gamma_in: Tensor[(config.hidden_size,), config.dtype],
            w_q: Tensor[(1, config.hidden_size, config.kda_proj), config.dtype],
            w_k: Tensor[(1, config.hidden_size, config.kda_proj), config.dtype],
            w_v: Tensor[(1, config.hidden_size, config.kda_proj), config.dtype],
            conv_w_q: Tensor[(config.short_conv_kernel_size, config.kda_proj), config.dtype],
            conv_w_k: Tensor[(config.short_conv_kernel_size, config.kda_proj), config.dtype],
            conv_w_v: Tensor[(config.short_conv_kernel_size, config.kda_proj), config.dtype],
            conv_state_q: Tensor[(1, _WS, config.kda_proj), config.dtype],
            conv_state_k: Tensor[(1, _WS, config.kda_proj), config.dtype],
            conv_state_v: Tensor[(1, _WS, config.kda_proj), config.dtype],
            w_f_a: Tensor[(1, config.hidden_size, config.kda_head_dim), config.dtype],
            w_f_b: Tensor[(1, config.kda_head_dim, config.kda_proj), config.dtype],
            dt_bias: Tensor[(config.kda_proj,), config.dtype],
            a_log: Tensor[(config.kda_num_heads,), config.dtype],
            w_b: Tensor[(1, config.hidden_size, config.kda_num_heads), config.dtype],
            w_g_a: Tensor[(1, config.hidden_size, config.kda_head_dim), config.dtype],
            w_g_b: Tensor[(1, config.kda_head_dim, config.kda_proj), config.dtype],
            gamma_o: Tensor[(config.kda_head_dim,), config.dtype],
            w_o: Tensor[(1, config.kda_proj, config.hidden_size), config.dtype],
            state: Tensor[
                (1, config.kda_num_heads, config.kda_head_dim, config.kda_head_dim), config.dtype
            ],
            scale: Tensor[(1, 1, 1), config.dtype],
        ):
    ...
    class KimiMoe:
        ...
        @func
        def router(
            tokens: Tensor[(S, config.hidden_size), config.dtype],
            w_router: Tensor[(config.hidden_size, _E), config.dtype],
            bias: Tensor[(_E,), config.dtype],
            routed_scale: Tensor[(1, 1), config.dtype],
        ):
        ...
        def shared_expert(
            tokens: Tensor[(S, config.hidden_size), config.dtype],
            w_gate: Tensor[(1, config.hidden_size, config.shared_intermediate), config.dtype],
            w_up: Tensor[(1, config.hidden_size, config.shared_intermediate), config.dtype],
            w_down: Tensor[(1, config.shared_intermediate, config.hidden_size), config.dtype],
        ) -> Tensor[(S, config.hidden_size), config.dtype]:
        ...
        def moe(
            hidden: Tensor[(1, S, config.hidden_size), config.dtype],
            gamma_post: Tensor[(config.hidden_size,), config.dtype],
            w_router: Tensor[(config.hidden_size, _E), config.dtype],
            bias: Tensor[(_E,), config.dtype],
            routed_scale: Tensor[(1, 1), config.dtype],
            w_gate: Tensor[(_E, config.moe_intermediate_size, config.hidden_size), config.dtype],
            w_up: Tensor[(_E, config.moe_intermediate_size, config.hidden_size), config.dtype],
            w_down: Tensor[(_E, config.hidden_size, config.moe_intermediate_size), config.dtype],
            sh_gate: Tensor[(1, config.hidden_size, config.shared_intermediate), config.dtype],
            sh_up: Tensor[(1, config.hidden_size, config.shared_intermediate), config.dtype],
            sh_down: Tensor[(1, config.shared_intermediate, config.hidden_size), config.dtype],
        ) -> Tensor[(1, S, config.hidden_size), config.dtype]:
            # Fused post-attention RMSNorm + MoE, no residual (the layer owns that).
            hn = tf.rms_norm(hidden, gamma_post, eps=_EPS)
...
#: The published model, at the published config. Any other is the caller's to
#: name: ``build_kimi_linear_48b_a3b(SMALL_MOE)``.
KimiLinear48BA3B = build_kimi_linear_48b_a3b(REAL)
