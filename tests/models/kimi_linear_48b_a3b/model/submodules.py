...
``tests.models.loader.load_model`` (see ``../submodules.py``).
...
- ``mla_attention`` reads ``ctx_len`` prior positions and returns this token's
  own key and value. The caller **appends**. Every shape is expressed in
...
class KimiMla:
    ...
    def mla_attention(
        ...
        k_cache: Tensor[(1, C, config.n_heads, config.qk_head_dim), config.dt],
        v_cache: Tensor[(1, C, config.n_heads, config.v_head_dim), config.dt],
        scale: Tensor[(1, 1, 1, 1), config.dt],
        w_o: Tensor[(1, config.v_proj, config.hidden), config.dt],
    ):
...
class KimiKda:
    ...
    @func
    def short_conv(
        x: Tensor[(1, S, config.kda_proj), config.dt],
        conv_w: Tensor[(config.short_conv, config.kda_proj), config.dt],
        conv_state: Tensor[(1, _WS, config.kda_proj), config.dt],
    ):
        ...
        acc = tf.reduce(
            tf.mul(window, tf.reshape(conv_w, new_shape=(1, config.short_conv, config.kda_proj))),
            axes=(1,),
            keepdim=True,
            kind="sum",
        )
        ...
        state_next = tf.slice(
            window,
            begin=(0, 1, 0),
            end=(1, config.short_conv, config.kda_proj),
            strides=(1, 1, 1),
        )
    ...
    @func
    def l2_normalize(
        x: Tensor[(1, S, config.kda_heads, config.kda_head_dim), config.dt],
    ) -> Tensor[(1, S, config.kda_heads, config.kda_head_dim), config.dt]:
    ...
    @func
    def kda_gate(
        hidden_norm: Tensor[(1, S, config.hidden), config.dt],
        w_f_a: Tensor[(1, config.hidden, config.kda_head_dim), config.dt],
        w_f_b: Tensor[(1, config.kda_head_dim, config.kda_proj), config.dt],
        dt_bias: Tensor[(config.kda_proj,), config.dt],
        a_log: Tensor[(config.kda_heads,), config.dt],
    ) -> Tensor[(1, S, config.kda_heads, config.kda_head_dim), config.dt]:
        ...
        g_raw = tf.reshape(
            ...
            new_shape=(1, S, config.kda_heads, config.kda_head_dim),
        )
        decay_rate = tf.neg(tf.exp(tf.reshape(a_log, new_shape=(1, 1, config.kda_heads, 1))))
    ...
    def kda_attention(
        hidden: Tensor[(1, S, config.hidden), config.dt],
        gamma_in: Tensor[(config.hidden,), config.dt],
        w_q: Tensor[(1, config.hidden, config.kda_proj), config.dt],
        w_k: Tensor[(1, config.hidden, config.kda_proj), config.dt],
        w_v: Tensor[(1, config.hidden, config.kda_proj), config.dt],
        conv_w_q: Tensor[(config.short_conv, config.kda_proj), config.dt],
        conv_w_k: Tensor[(config.short_conv, config.kda_proj), config.dt],
        conv_w_v: Tensor[(config.short_conv, config.kda_proj), config.dt],
        conv_state_q: Tensor[(1, _WS, config.kda_proj), config.dt],
        conv_state_k: Tensor[(1, _WS, config.kda_proj), config.dt],
        conv_state_v: Tensor[(1, _WS, config.kda_proj), config.dt],
        w_f_a: Tensor[(1, config.hidden, config.kda_head_dim), config.dt],
        w_f_b: Tensor[(1, config.kda_head_dim, config.kda_proj), config.dt],
        dt_bias: Tensor[(config.kda_proj,), config.dt],
        a_log: Tensor[(config.kda_heads,), config.dt],
        w_b: Tensor[(1, config.hidden, config.kda_heads), config.dt],
        w_g_a: Tensor[(1, config.hidden, config.kda_head_dim), config.dt],
        w_g_b: Tensor[(1, config.kda_head_dim, config.kda_proj), config.dt],
        gamma_o: Tensor[(config.kda_head_dim,), config.dt],
        w_o: Tensor[(1, config.kda_proj, config.hidden), config.dt],
        state: Tensor[
            (1, config.kda_heads, config.kda_head_dim, config.kda_head_dim), config.dt
        ],
        scale: Tensor[(1, 1, 1), config.dt],
    ):
...
class KimiMoe:
    ...
    Routing has one subtlety that no shape check catches. Selection reads
    ...
    @func
    def router(
        tokens: Tensor[(S, config.hidden), config.dt],
        w_router: Tensor[(config.hidden, config.n_experts), config.dt],
        bias: Tensor[(config.n_experts,), config.dt],
        routed_scale: Tensor[(1, 1), config.dt],
    ):
        ...
        top_biased, indices = tf.topk(biased, k=config.top_k, axis=-1)
        ...
        return tf.cast(weights, dtype=config.dt), indices

    @func
    def shared_expert(
        tokens: Tensor[(S, config.hidden), config.dt],
        w_gate: Tensor[(1, config.hidden, config.shared_intermediate), config.dt],
        w_up: Tensor[(1, config.hidden, config.shared_intermediate), config.dt],
        w_down: Tensor[(1, config.shared_intermediate, config.hidden), config.dt],
    ) -> Tensor[(S, config.hidden), config.dt]:
        # One dense SwiGLU expert every token pays for, unscaled: the routed
        # scaling factor applies to the routed branch only.
        x = tf.reshape(tokens, new_shape=(1, S, config.hidden))
        gate = tf.matmul(x, w_gate)
        up = tf.matmul(x, w_up)
        h = tf.mul(tf.mul(gate, tf.sigmoid(gate)), up)
        return tf.reshape(tf.matmul(h, w_down), new_shape=(S, config.hidden))
    ...
    def moe(
        ...
        bias: Tensor[(config.n_experts,), config.dt],
        routed_scale: Tensor[(1, 1), config.dt],
        w_gate: Tensor[(config.n_experts, config.moe_intermediate, config.hidden), config.dt],
        w_up: Tensor[(config.n_experts, config.moe_intermediate, config.hidden), config.dt],
        w_down: Tensor[(config.n_experts, config.hidden, config.moe_intermediate), config.dt],
        sh_gate: Tensor[(1, config.hidden, config.shared_intermediate), config.dt],
        sh_up: Tensor[(1, config.hidden, config.shared_intermediate), config.dt],
        sh_down: Tensor[(1, config.shared_intermediate, config.hidden), config.dt],
        ...
        tokens = tf.reshape(hn, new_shape=(S, config.hidden))
        ...
        tok4 = tf.reshape(tokens, new_shape=(S, 1, config.hidden, 1))
        ...
        down = tf.reshape(tf.matmul(d_sel, h4), new_shape=(S, _TOPK, config.hidden))
        routed = tf.reduce(
        ...
        return tf.reshape(tf.add(routed, shared), new_shape=(1, S, config.hidden))
