...
def generate(m, input_ids, max_new_tokens: int, *, device: str = "cuda"):
    ...
    A step hands back its own fresh per-layer entry; growing the cache with it is
    this loop's step, through the model's ``append_cache``."""
    _require_hook(m, "init_caches")
    _require_hook(m, "prepare_inputs_for_generation")
    _require_hook(m, "append_cache")
    ...
    for t in range(max_new_tokens):
        ...
        logits, fresh = m(*args)
        past_key_values = m.append_cache(past_key_values, fresh)
